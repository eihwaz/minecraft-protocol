...
impl ServerBoundGamePacket {
    pub fn get_type_id(&self) -> u8 {
        match self {
            ...
            Self::PickItem(_) => 0x18,
            Self::NameItem(_) => 0x20,
            Self::SelectTrade(_) => 0x23,
            Self::SetBeaconEffect(_) => 0x24,
            Self::UpdateCommandBlock(_) => 0x26,
            Self::UpdateCommandBlockMinecart(_) => 0x27,
            Self::UpdateStructureBlock(_) => 0x2A,
            ...
            Self::GenerateStructure(_) => 0x0F,
            Self::ServerBoundKeepAlive(_) => 0x10,
            Self::LockDifficulty(_) => 0x11,
            Self::ServerBoundPosition(_) => 0x12,
            Self::PositionLook(_) => 0x13,
            Self::Look(_) => 0x14,
            Self::Flying(_) => 0x15,
            Self::ServerBoundVehicleMove(_) => 0x16,
            Self::SteerBoat(_) => 0x17,
            Self::CraftRecipeRequest(_) => 0x19,
            Self::ServerBoundAbilities(_) => 0x1A,
            Self::BlockDig(_) => 0x1B,
            Self::EntityAction(_) => 0x1C,
            Self::SteerVehicle(_) => 0x1D,
            Self::DisplayedRecipe(_) => 0x1E,
            Self::RecipeBook(_) => 0x1F,
            Self::ResourcePackReceive(_) => 0x21,
            Self::ServerBoundHeldItemSlot(_) => 0x25,
            Self::SetCreativeSlot(_) => 0x28,
            Self::UpdateJigsawBlock(_) => 0x29,
            Self::UpdateSign(_) => 0x2B,
            Self::ArmAnimation(_) => 0x2C,
            Self::Spectate(_) => 0x2D,
            Self::BlockPlace(_) => 0x2E,
            Self::UseItem(_) => 0x2F,
            Self::AdvancementTab(_) => 0x22,
        }
    }
    ...
    pub fn decode<R: Read>(type_id: u8, reader: &mut R) -> Result<Self, DecodeError> {
        match type_id {
            ...
            0x0F => {
                let generate_structure = GenerateStructure::decode(reader)?;

                Ok(Self::GenerateStructure(generate_structure))
            }
            0x10 => {
            ...
            0x1E => {
                let displayed_recipe = DisplayedRecipe::decode(reader)?;

                Ok(Self::DisplayedRecipe(displayed_recipe))
            }
            0x1F => {
                let recipe_book = RecipeBook::decode(reader)?;

                Ok(Self::RecipeBook(recipe_book))
            }
            0x21 => {
    ...
    pub fn use_entity(target: i32, mouse: i32, sneaking: bool) -> Self {
        let use_entity = UseEntity {
            target,
            mouse,
            sneaking,
        };
    ...
    pub fn generate_structure(location: Position, levels: i32, keep_jigsaws: bool) -> Self {
        let generate_structure = GenerateStructure {
            location,
            levels,
            keep_jigsaws,
        };
    ...
    pub fn server_bound_abilities(flags: i8) -> Self {
        let server_bound_abilities = ServerBoundAbilities { flags };
    ...
    pub fn displayed_recipe(recipe_id: String) -> Self {
        let displayed_recipe = DisplayedRecipe { recipe_id };

        Self::DisplayedRecipe(displayed_recipe)
    }

    pub fn recipe_book(book_id: i32, book_open: bool, filter_active: bool) -> Self {
...
impl ClientBoundGamePacket {
    pub fn get_type_id(&self) -> u8 {
        match self {
            ...
            Self::SpawnEntityLiving(_) => 0x02,
            Self::SpawnEntityPainting(_) => 0x03,
            Self::NamedEntitySpawn(_) => 0x04,
            Self::Animation(_) => 0x05,
            Self::Statistics => 0x06,
            ...
            Self::ClientBoundTabComplete(_) => 0x0F,
            Self::DeclareCommands(_) => 0x10,
            Self::FacePlayer(_) => 0x33,
            Self::NbtQueryResponse(_) => 0x54,
            Self::ClientBoundChat(_) => 0x0E,
            Self::MultiBlockChange(_) => 0x3B,
            Self::ClientBoundTransaction(_) => 0x11,
            Self::ClientBoundCloseWindow(_) => 0x12,
            Self::OpenWindow(_) => 0x2D,
            Self::WindowItems(_) => 0x13,
            Self::CraftProgressBar(_) => 0x14,
            Self::SetSlot(_) => 0x15,
            Self::SetCooldown(_) => 0x16,
            Self::ClientBoundCustomPayload(_) => 0x17,
            Self::NamedSoundEffect(_) => 0x18,
            Self::KickDisconnect(_) => 0x19,
            Self::EntityStatus(_) => 0x1A,
            Self::Explosion(_) => 0x1B,
            Self::UnloadChunk(_) => 0x1C,
            Self::GameStateChange(_) => 0x1D,
            Self::OpenHorseWindow(_) => 0x1E,
            Self::ClientBoundKeepAlive(_) => 0x1F,
            Self::MapChunk(_) => 0x20,
            Self::WorldEvent(_) => 0x21,
            Self::WorldParticles(_) => 0x22,
            Self::UpdateLight(_) => 0x23,
            Self::JoinGame(_) => 0x24,
            Self::Map(_) => 0x25,
            Self::TradeList(_) => 0x26,
            Self::RelEntityMove(_) => 0x27,
            Self::EntityMoveLook(_) => 0x28,
            Self::EntityLook(_) => 0x29,
            Self::Entity(_) => 0x2A,
            Self::ClientBoundVehicleMove(_) => 0x2B,
            Self::OpenBook(_) => 0x2C,
            Self::OpenSignEntity(_) => 0x2E,
            Self::CraftRecipeResponse(_) => 0x2F,
            Self::ClientBoundAbilities(_) => 0x30,
            Self::CombatEvent(_) => 0x31,
            Self::PlayerInfo(_) => 0x32,
            Self::ClientBoundPosition(_) => 0x34,
            Self::UnlockRecipes(_) => 0x35,
            Self::EntityDestroy => 0x36,
            Self::RemoveEntityEffect(_) => 0x37,
            Self::ResourcePackSend(_) => 0x38,
            Self::Respawn(_) => 0x39,
            Self::EntityHeadRotation(_) => 0x3A,
            Self::WorldBorder(_) => 0x3D,
            Self::Camera(_) => 0x3E,
            Self::ClientBoundHeldItemSlot(_) => 0x3F,
            Self::UpdateViewPosition(_) => 0x40,
            Self::UpdateViewDistance(_) => 0x41,
            Self::ScoreboardDisplayObjective(_) => 0x43,
            Self::EntityMetadata(_) => 0x44,
            Self::AttachEntity(_) => 0x45,
            Self::EntityVelocity(_) => 0x46,
            Self::EntityEquipment(_) => 0x47,
            Self::Experience(_) => 0x48,
            Self::UpdateHealth(_) => 0x49,
            Self::ScoreboardObjective(_) => 0x4A,
            Self::SetPassengers(_) => 0x4B,
            Self::Teams(_) => 0x4C,
            Self::ScoreboardScore(_) => 0x4D,
            ...
            Self::Tags(_) => 0x5B,
            Self::AcknowledgePlayerDigging(_) => 0x07,
        }
    }
    ...
    pub fn client_bound_chat(message: String, position: i8, sender: Uuid) -> Self {
        let client_bound_chat = ClientBoundChat {
            message,
            position,
            sender,
        };
    ...
    pub fn multi_block_change(not_trust_edges: bool) -> Self {
        let multi_block_change = MultiBlockChange { not_trust_edges };
    ...
    pub fn join_game(
        ...
        dimension_codec: CompoundTag,
        dimension: CompoundTag,
        world_name: String,
        hashed_seed: i64,
        max_players: i32,
        view_distance: i32,
    ...
    pub fn entity_equipment(entity_id: i32) -> Self {
        let entity_equipment = EntityEquipment { entity_id };
