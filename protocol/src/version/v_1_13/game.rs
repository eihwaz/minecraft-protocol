...
impl ServerBoundGamePacket {
    pub fn get_type_id(&self) -> u8 {
        match self {
            ...
            Self::EditBook(_) => 0x0B,
            Self::QueryEntityNbt(_) => 0x0C,
            Self::PickItem(_) => 0x15,
            Self::NameItem(_) => 0x1C,
            Self::SelectTrade(_) => 0x1F,
            Self::SetBeaconEffect(_) => 0x20,
            Self::UpdateCommandBlock(_) => 0x22,
            Self::UpdateCommandBlockMinecart(_) => 0x23,
            Self::UpdateStructureBlock(_) => 0x25,
            Self::ServerBoundTabComplete(_) => 0x05,
            Self::ServerBoundChat(_) => 0x02,
            Self::ClientCommand(_) => 0x03,
            Self::Settings(_) => 0x04,
            Self::ServerBoundTransaction(_) => 0x06,
            Self::EnchantItem(_) => 0x07,
            Self::WindowClick(_) => 0x08,
            Self::ServerBoundCloseWindow(_) => 0x09,
            Self::ServerBoundCustomPayload(_) => 0x0A,
            Self::UseEntity(_) => 0x0D,
            Self::ServerBoundKeepAlive(_) => 0x0E,
            Self::ServerBoundPosition(_) => 0x10,
            Self::PositionLook(_) => 0x11,
            Self::Look(_) => 0x12,
            Self::Flying(_) => 0x0F,
            Self::ServerBoundVehicleMove(_) => 0x13,
            Self::SteerBoat(_) => 0x14,
            Self::CraftRecipeRequest(_) => 0x16,
            Self::ServerBoundAbilities(_) => 0x17,
            Self::BlockDig(_) => 0x18,
            Self::EntityAction(_) => 0x19,
            Self::SteerVehicle(_) => 0x1A,
            Self::CraftingBookData(_) => 0x1B,
            Self::ResourcePackReceive(_) => 0x1D,
            Self::ServerBoundHeldItemSlot(_) => 0x21,
            Self::SetCreativeSlot(_) => 0x24,
            Self::UpdateSign(_) => 0x26,
            Self::ArmAnimation(_) => 0x27,
            Self::Spectate(_) => 0x28,
            Self::BlockPlace(_) => 0x29,
            Self::UseItem(_) => 0x2A,
            Self::AdvancementTab(_) => 0x1E,
        }
    }
    ...
    pub fn edit_book(new_book: Option<Slot>, signing: bool) -> Self {
        let edit_book = EditBook { new_book, signing };
...
impl ClientBoundGamePacket {
    pub fn get_type_id(&self) -> u8 {
        match self {
            ...
            Self::FacePlayer(_) => 0x31,
            Self::NbtQueryResponse(_) => 0x1D,
            ...
            Self::OpenWindow(_) => 0x14,
            Self::WindowItems(_) => 0x15,
            Self::CraftProgressBar(_) => 0x16,
            Self::SetSlot(_) => 0x17,
            Self::SetCooldown(_) => 0x18,
            Self::ClientBoundCustomPayload(_) => 0x19,
            Self::NamedSoundEffect(_) => 0x1A,
            Self::KickDisconnect(_) => 0x1B,
            Self::EntityStatus(_) => 0x1C,
            Self::Explosion(_) => 0x1E,
            Self::UnloadChunk(_) => 0x1F,
            Self::GameStateChange(_) => 0x20,
            Self::ClientBoundKeepAlive(_) => 0x21,
            Self::MapChunk(_) => 0x22,
            Self::WorldEvent(_) => 0x23,
            Self::WorldParticles(_) => 0x24,
            ...
            Self::OpenSignEntity(_) => 0x2C,
            Self::CraftRecipeResponse(_) => 0x2D,
            Self::ClientBoundAbilities(_) => 0x2E,
            Self::CombatEvent(_) => 0x2F,
            Self::PlayerInfo(_) => 0x30,
            Self::ClientBoundPosition(_) => 0x32,
            Self::Bed(_) => 0x33,
            Self::UnlockRecipes(_) => 0x34,
            Self::EntityDestroy => 0x35,
            Self::RemoveEntityEffect(_) => 0x36,
            Self::ResourcePackSend(_) => 0x37,
            Self::Respawn(_) => 0x38,
            Self::EntityHeadRotation(_) => 0x39,
            Self::WorldBorder(_) => 0x3B,
            Self::Camera(_) => 0x3C,
            Self::ClientBoundHeldItemSlot(_) => 0x3D,
            Self::ScoreboardDisplayObjective(_) => 0x3E,
            Self::EntityMetadata(_) => 0x3F,
            Self::AttachEntity(_) => 0x40,
            Self::EntityVelocity(_) => 0x41,
            Self::EntityEquipment(_) => 0x42,
            Self::Experience(_) => 0x43,
            Self::UpdateHealth(_) => 0x44,
            Self::ScoreboardObjective(_) => 0x45,
            Self::SetPassengers(_) => 0x46,
            Self::Teams(_) => 0x47,
            Self::ScoreboardScore(_) => 0x48,
            Self::SpawnPosition(_) => 0x49,
            Self::UpdateTime(_) => 0x4A,
            Self::Title(_) => 0x4B,
            Self::StopSound(_) => 0x4C,
            Self::SoundEffect(_) => 0x4D,
            Self::PlayerlistHeader(_) => 0x4E,
            Self::Collect(_) => 0x4F,
            Self::EntityTeleport(_) => 0x50,
            Self::EntityUpdateAttributes(_) => 0x52,
            Self::EntityEffect(_) => 0x53,
            Self::SelectAdvancementTab(_) => 0x3A,
            Self::DeclareRecipes => 0x54,
            Self::Tags(_) => 0x55,
        }
    }
    ...
    pub fn tags(block_tags: TagsMap, item_tags: TagsMap, fluid_tags: TagsMap) -> Self {
        let tags = Tags {
            block_tags,
            item_tags,
            fluid_tags,
