...
impl ClientBoundGamePacket {
    pub fn get_type_id(&self) -> u8 {
        match self {
            ...
            Self::ClientBoundTabComplete(_) => 0x11,
            Self::DeclareCommands(_) => 0x12,
            Self::FacePlayer(_) => 0x35,
            Self::NbtQueryResponse(_) => 0x55,
            Self::ClientBoundChat(_) => 0x0F,
            Self::MultiBlockChange(_) => 0x10,
            Self::ClientBoundTransaction(_) => 0x13,
            Self::ClientBoundCloseWindow(_) => 0x14,
            Self::OpenWindow(_) => 0x2F,
            ...
            Self::OpenHorseWindow(_) => 0x20,
            Self::ClientBoundKeepAlive(_) => 0x21,
            Self::MapChunk(_) => 0x22,
            Self::WorldEvent(_) => 0x23,
            Self::WorldParticles(_) => 0x24,
            Self::UpdateLight(_) => 0x25,
            Self::JoinGame(_) => 0x26,
            Self::Map(_) => 0x27,
            Self::TradeList(_) => 0x28,
            Self::RelEntityMove(_) => 0x29,
            Self::EntityMoveLook(_) => 0x2A,
            Self::EntityLook(_) => 0x2B,
            Self::Entity(_) => 0x2C,
            Self::ClientBoundVehicleMove(_) => 0x2D,
            Self::OpenBook(_) => 0x2E,
            Self::OpenSignEntity(_) => 0x30,
            Self::CraftRecipeResponse(_) => 0x31,
            Self::ClientBoundAbilities(_) => 0x32,
            Self::CombatEvent(_) => 0x33,
            Self::PlayerInfo(_) => 0x34,
            Self::ClientBoundPosition(_) => 0x36,
            Self::UnlockRecipes(_) => 0x37,
            Self::EntityDestroy => 0x38,
            Self::RemoveEntityEffect(_) => 0x39,
            Self::ResourcePackSend(_) => 0x3A,
            Self::Respawn(_) => 0x3B,
            Self::EntityHeadRotation(_) => 0x3C,
            Self::WorldBorder(_) => 0x3E,
            Self::Camera(_) => 0x3F,
            Self::ClientBoundHeldItemSlot(_) => 0x40,
            Self::UpdateViewPosition(_) => 0x41,
            Self::UpdateViewDistance(_) => 0x42,
            Self::ScoreboardDisplayObjective(_) => 0x44,
            Self::EntityMetadata(_) => 0x45,
            Self::AttachEntity(_) => 0x46,
            Self::EntityVelocity(_) => 0x47,
            Self::EntityEquipment(_) => 0x48,
            Self::Experience(_) => 0x49,
            Self::UpdateHealth(_) => 0x4A,
            Self::ScoreboardObjective(_) => 0x4B,
            Self::SetPassengers(_) => 0x4C,
            Self::Teams(_) => 0x4D,
            Self::ScoreboardScore(_) => 0x4E,
            Self::SpawnPosition(_) => 0x43,
            Self::UpdateTime(_) => 0x4F,
            Self::Title(_) => 0x50,
            Self::EntitySoundEffect(_) => 0x51,
            Self::StopSound(_) => 0x53,
            Self::SoundEffect(_) => 0x52,
            Self::PlayerlistHeader(_) => 0x54,
            Self::Collect(_) => 0x56,
            Self::EntityTeleport(_) => 0x57,
            Self::EntityUpdateAttributes(_) => 0x59,
            Self::EntityEffect(_) => 0x5A,
            Self::SelectAdvancementTab(_) => 0x3D,
            Self::DeclareRecipes => 0x5B,
            Self::Tags(_) => 0x5C,
            Self::AcknowledgePlayerDigging(_) => 0x08,
        }
    }
    ...
    pub fn respawn(dimension: i32, hashed_seed: i64, gamemode: u8, level_type: String) -> Self {
        let respawn = Respawn {
            dimension,
            hashed_seed,
            gamemode,
